...
# # ---------------------------------------------------
# # Ensure src is in Python path for sibling imports
# # ---------------------------------------------------
# sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

# from model_inference import predict
# from train import get_feature_importance
# from preprocessing import transform_preprocessor, encode_plant_age, clean_column_names
...
#     datefmt="%Y-%m-%d %H:%M:%S",
# )
# logger = logging.getLogger("CAPEX_Estimation_API")
...
#     version="1.0",
#     description="API for CAPEX prediction using ML pipeline and FastAPI."
# )
...
#     Estimated_CAPEX: Optional[float] = None  # optional for inference
...
# @app.get("/")
# def root():
#     return {"message": "CAPEX Estimation API is running. Check /docs for API endpoints."}
...
#         logger.info(f"Prediction successful → {float(y_pred[0])}")
#         return {"predicted_CAPEX": float(y_pred[0])}
#     except Exception as e:
...
#         logger.info(f"Batch prediction successful → {len(preds)} rows")
...
#     try:
#         top_features = get_feature_importance(model, feature_order, top_n=n)
#         logger.info(f"Returning top {n} features by importance")
#         return {"top_features": top_features.to_dict(orient="records")}
#     except Exception as e:
...
# ---------------------------------------------------
# Input Schema (User-friendly names kept)
# ---------------------------------------------------
class InputData(BaseModel):
